...
router.get("/overview", async (req, res) => {
  try {
    const [activeJourneys, activeAlerts, recentAlerts] = await Promise.all([
      Journey.find({ status: { $in: ["ACTIVE", "DEVIATED", "SOS"] } })
    ...
    res.json({
      ...
      summary: {
        ...
        sosAlerts: activeAlerts.filter((alert) => alert.type === "SOS").length,
        deviations: activeAlerts.filter((alert) => alert.type === "ROUTE_DEVIATION").length,
      },
      activeJourneys,
      activeAlerts,
      recentAlerts,
...
router.get("/journeys", async (req, res) => {
  try {
    const journeys = await Journey.find({
      status: { $in: ["ACTIVE", "DEVIATED", "SOS"] },
    }).sort({ lastLocationAt: -1 }).limit(200).lean();
